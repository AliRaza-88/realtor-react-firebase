...
useEffect(()=>{
  ...
  function pathMatchRoute(route) {
    const isMatch = route === location.pathname;
    console.log(`Route: ${route}, Current Pathname: ${location.pathname}, Is Match: ${isMatch}`);
    return isMatch;
  }
          ...
          <ul className="flex space-x-10">
            <li
              className={`py-3 text-sm font-semibold text-gray-400 border-b-[3px] border-b-transparent ${
                pathMatchRoute("/") && "text-black border-b-red-600"} cursor-pointer`}
              onClick={() => navigate("/")}
            >
              Home
            </li>
